...
from newton_raphson_enhanced_utils.jax_utils import jit
...
def generate_reference_trajectory(
    ...
    def one_sample(t: jnp.float64): # type: ignore
        pos, vel = traj_func(t, ctx)   # pos: [x, y, z, yaw], vel: [vx, vy, vz, yaw_rate]

        return pos, vel

    # Vectorize over all sample times; ctx stays constant (static) for your jitted traj funcs
    pos_jax, vel_jax = jax.vmap(one_sample)(t_samples)         # shape: (num_steps, 12)
    # print(f"Generated trajectory: {pos_jax}")
    return pos_jax, vel_jax